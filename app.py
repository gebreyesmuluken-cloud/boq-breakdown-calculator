...
def calculate_article(article_row: pd.Series, breakdown_df: pd.DataFrame):
    ...
    def close_group(end_idx: int):
        ...
        if current_group_start is None:
            return
        subtotal = (
            pd.to_numeric(work.loc[current_group_start + 1 : end_idx, "Total Cost"], errors="coerce")
            .fillna(0.0)
            .sum()
        )
    ...
    def close_total(end_idx: int):
        ...
        if current_total_start is None:
            return
        subtotal = (
            pd.to_numeric(work.loc[current_total_start + 1 : end_idx, "Total Cost"], errors="coerce")
            .fillna(0.0)
            .sum()
        )
...
top_info_cols[3].markdown(
    f"<div class='small-note'>Edit mode: <b>{'ON' if st.session_state.edit_mode else 'OFF'}</b>. Type <b>+</b> in one Open cell to open that article breakdown.</div>",
    unsafe_allow_html=True,
)
...
edited_boq = st.data_editor(
    ...
    column_config={
        ...
        "Template_Name": st.column_config.SelectboxColumn("Template_Name", options=template_options),
    },
)
...
if selected_article:
    ...
    if not article_row.empty:
        ...
        st.markdown(
            f"<div class='active-article'><b>Active article:</b> {selected_article} | {article['Description']} | Quantity: {article['Quantity']} {article['Unit']}</div>",
            unsafe_allow_html=True,
        )

        summary_cols = st.columns([1.2, 1.2, 1.6, 1.0, 1.0])
        ...
        if summary_cols[3].button("Load Template", use_container_width=True):
            if str(article["Template_Name"]).strip():
                load_template(selected_article, article["Template_Name"])
                st.success(f"Template loaded for {selected_article}.")
            else:
                st.warning("Select a template name in the BOQ sheet first.")

        if summary_cols[4].button("Close", use_container_width=True):
            st.session_state.selected_article = None
            st.rerun()
        ...
        st.markdown(
            "<div class='small-note'>Breakdown columns: Type, Category, Code, Description, Norm, Formula, Resultant, Quantity, Unit, Unit Price, Total Cost</div>",
            unsafe_allow_html=True,
        )
...
library_expander = st.expander("Breakdown Library Sheet")
